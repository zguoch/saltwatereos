...
def setup(app):
    ...
    app.add_config_value('plot_formats', ['svg', 'jpg', 'pdf'], True)
...
def get_plot_formats(config):
    default_dpi = {'png': 80, 'hires.png': 200, 'pdf': 200, 'jpg':400}
    formats = []
    ...
    for i, code_piece in enumerate(code_pieces):
        ...
        for j, figman in enumerate(fig_managers):
            ...
            for fmt, dpi in formats:
                try:
                    if(fmt in ['pdf','svg']):
                        plt.savefig(img.filename(fmt),bbox_inches='tight')
                    else:
                        plt.savefig(img.filename(fmt), dpi=dpi, bbox_inches='tight')
